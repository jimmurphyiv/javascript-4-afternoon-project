...
function Employee(name, email, hireDate) {
  ...
  */
  ...
  let twister = new Movie('twister', 'shitty', 6.9);
  console.log(twister.changeRating(7.8));
  ////////// PROBLEM 5 //////////
  
  // Write a constructor function called User. This function should take in 4 parameters called name, age, email, and savedPosts
  // in that order. Name and email will be strings, age will be a number and savedPosts will be an array of objects. 
  ...
  // Once the User constructor function is created, write a prototype method for the User function. Name this method addSavedPost. 
  //It should take in three parameters: id (a number), title (a string) and rating (a number). Use these parameters to 
  //create a new object and add it to the savedPosts array. Make sure to name the properties the same as described previously
  // (id, title, rating).
  ...
  let b = new User('vv', 3, '33', ['3', 3, 4]);
  b.addSavedPost(3, 'thanks', 4);
  
  ////////// PROBLEM 6 //////////
  
  // You will be using the constructor function you just created in problem 5.
  // Write a prototype method for the User constructor function named removeSavedPost that will take in one number parameter
  ...
  let index = this.savedPosts.findIndex(e => {
    return e.id === id
  });
  ...
  //Use the id to find the matching object in the savedPosts array. Once you find the matching object, 
  //update it's rating score with the new rating parameter.
  
  // Code here
  User.prototype.changePostRating = function (id, rating) {
   let index = this.savedPosts.findIndex(element => {
      return element.id === id
    })
    this.savedPosts[index].rating = rating
    console.log((this.savedPosts))
  }
  b.changePostRating(33, 420);